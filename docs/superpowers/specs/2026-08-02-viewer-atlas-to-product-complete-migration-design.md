...
关键约束：已经正确的 40 px 顶栏、24 px 目录行、52 px 折叠轨道和选择摘要不能回退。内容网格卡片与选中边界现由 `2026-08-06-viewer-square-image-card-selection-design.md` 管理：卡片使用直角，选中框包围包含文件名的完整卡片，不再使用缩略图内部选中框。
