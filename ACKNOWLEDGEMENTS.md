...
- [Rust](https://github.com/rust-lang/rust) and Cargo — native application language and build toolchain. The direct Rust crates currently include `async-trait`, the `objc2` Core Foundation/Core Graphics/Image I/O bindings, `serde`, `serde_json` (tests), `tempfile` (tests), `thiserror`, and `uuid`.
