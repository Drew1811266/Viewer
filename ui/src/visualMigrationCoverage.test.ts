...
describe('atlas-to-product migration coverage', () => {
  it('tracks every audited state exactly once', () => {
    ...
    expect(audit).toHaveLength(92)
    expect(new Set(audit).size).toBe(92)
    expect(ledger).toHaveLength(92)
    expect(new Set(ledger).size).toBe(92)
  ...
  it('records automated evidence for every state before native acceptance', () => {
    ...
    expect(ledgerRows).toHaveLength(92)
