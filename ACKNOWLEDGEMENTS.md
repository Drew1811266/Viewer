...
- [Rust](https://github.com/rust-lang/rust) and Cargo — native application language and build toolchain. The direct Rust crates currently include `async-trait`, `block2`, the `objc2` Foundation/Core Foundation/Core Graphics/Image I/O/Quick Look Thumbnailing bindings, `serde`, `serde_json` (tests), `tempfile` (tests), `thiserror`, `tokio`, and `uuid`.
