...
| 3 | Cancelled Finder drag | An image card-body export was cancelled with Escape before release. `destinations/cancelled-drag/` remained empty; all four sources retained the mapped hashes; `organization-move/` and `organization-copy/` remained empty before organization testing. **PASS**. |
