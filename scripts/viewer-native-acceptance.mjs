...
export { combinePngEvidence, readRgbaPng, writeRgbaPng }
...
export function buildStateEntryPlan(id) {
  ...
  const plans = {
    ...
    'OTH-01': [
      ...
      { kind: 'click', target: { name: '商品-03.jpg' }, modifiers: ['command'] },
      { kind: 'movePointerToTitlebar' },
      { kind: 'assert', target: { role: 'AXButton', name: '其它文件 · 3' } },
    ],
    'OTH-02': [
      { kind: 'prepareFixture', operation: 'prepareOrganizationDrag' },
      ...openContent('紧凑'),
    ...
    'PRE-05': [
      ...
      ...openFolder('衣服/A01'),
      { kind: 'click', target: image('加载中.png') },
    ...
    'DOC-02': [
      ...openDocumentFolder(),
      { kind: 'click', target: { name: 'plain.txt' } },
      { kind: 'key', key: 'space', modifiers: [] },
      { kind: 'assert', target: { role: 'AXButton', name: '关闭预览' } },
    ],
    'DOC-03': [
      ...openDocumentFolder(),
      { kind: 'click', target: { name: 'gb18030.txt' } },
      { kind: 'key', key: 'space', modifiers: [] },
      { kind: 'assert', target: { role: 'AXHeading', name: '需要选择文本编码' } },
    ],
    'DOC-04': [
      ...openDocumentFolder(),
      { kind: 'click', target: { name: 'large.txt' } },
      { kind: 'key', key: 'space', modifiers: [] },
    ...
    'DOC-06': [
      ...openFolder('其它'),
      { kind: 'click', target: { name: 'unsupported.bin' } },
      { kind: 'key', key: 'space', modifiers: [] },
