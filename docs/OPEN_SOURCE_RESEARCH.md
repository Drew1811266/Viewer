...
qView、nomacs、digiKam 和 TagStudio 使用 GPL 系列许可证。在 Viewer 自身许可证未确定前：
...
初步推荐 Viewer 使用 **Apache-2.0**。最终选择应在复制任何第三方源码之前完成。

## 7. 下一步审计清单

1. 确定 Viewer 自身许可证。
2. 为每个候选依赖建立版本、许可证、维护状态和替代方案表。
3. 在技术验证阶段分别制作网格虚拟化、文件监听、废纸篓、Unicode 模糊搜索和 Image I/O 缩略图原型。
4. 只在原型通过性能与异常测试后将框架或依赖纳入正式技术栈。
5. 发布 Viewer 0.1 前生成最终第三方声明并人工复核。
