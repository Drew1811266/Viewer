...
describe('workspace style contracts', () => {
  it('keeps document scrolling locked so only the workspace owns vertical scrolling', () => {
    const documentRoot = appCss.match(/html,\s*body,\s*#root\s*\{([^}]*)\}/)?.[1]

    expect(documentRoot).toMatch(/\boverflow:\s*hidden;/)
    expect(documentRoot).toMatch(/\boverscroll-behavior:\s*none;/)
  })
